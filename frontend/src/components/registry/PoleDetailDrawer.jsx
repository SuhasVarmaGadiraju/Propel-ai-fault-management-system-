                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 text-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-slate-700">
                    <div>
                      ...
                      <span className="font-medium text-slate-900">{poleData.feeder?.feeder_code}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block">Transformer (DT)</span>
                      <span className="font-medium text-slate-900">{poleData.transformer?.transformer_code}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-medium text-slate-900">{poleData.pole_type}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 text-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-slate-700">
                    <div>
                      ...
                      <span className="font-mono text-slate-900">{poleData.latitude}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block">Longitude</span>
                      <span className="font-mono text-slate-900">{poleData.longitude}</span>
                    </div>
                    <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between p-2.5 bg-white border border-slate-200 rounded-lg">
                      ...
                      <span className="font-semibold text-slate-900">
                        {poleData.parent_pole ? poleData.parent_pole.pole_code : 'None (Root Node)'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-2.5 bg-white border border-slate-200 rounded-lg">
                      <span className="text-slate-500">Sequence On Line</span>
                      <span className="font-semibold text-slate-900">
                        {poleData.seq_on_line !== null ? poleData.seq_on_line : 'N/A'}
                      </span>
                    </div>
                    <div className="p-2.5 bg-white border border-slate-200 rounded-lg">
                      <span className="text-slate-500 block mb-1">Downstream Branch Poles ({poleData.children_poles?.length || 0})</span>
                      {poleData.children_poles && poleData.children_poles.length > 0 ? (
                  ...
                  <div className="bg-brand-50/50 border border-brand-200 rounded-xl p-4 space-y-3 text-xs">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-slate-700">
                      <div>
                        ...
                        <span className="font-mono font-bold text-brand-700">{poleData.device.device_id}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block">Firmware</span>
                        <span className="font-mono text-slate-900">{poleData.device.firmware_version}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block">Battery Voltage</span>
                        <span className="font-mono text-slate-900">{poleData.device.battery_mv} mV</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block">Signal RSSI</span>
                        <span className="font-mono text-slate-900">{poleData.device.last_rssi} dBm</span>
                      </div>
                    </div>
                  </div>
                )}
